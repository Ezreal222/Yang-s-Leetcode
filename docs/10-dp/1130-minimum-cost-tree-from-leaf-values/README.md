...
6. **替代解法: O(n) 单调栈贪心 / Greedy monotonic stack alternative**
    ...
    本题有 O(n) 单调栈解, 单独放在 [§11 单调栈 · 1130](../../11-monotonic-stack/1130-minimum-cost-tree-from-leaf-values/README.md). 思想是"小叶子早点跟最小的较大邻居配对一次, 然后被吃掉" — 单调递减栈维护.

    DP 是通解, 单调栈是该特定结构的 hack. **先学透区间 DP 再去看栈解会更顺**.
